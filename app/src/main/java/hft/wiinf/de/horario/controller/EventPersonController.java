...
public class EventPersonController {
    ...
    public static int getNumberOfPendingEventsForPerson(Person person) {
        deleteExpiredPendingEventsForPerson(person);
        return new Select().from(EventPerson.class).where("person = ?", person.getId()).and("status = ?", AcceptedState.WAITING).execute().size();
    }
}
